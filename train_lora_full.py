# менее чистый скрипт обучения через lora, но
# здесь есть класс валидации, который нужно чуть донастроить
...
path_finetuned_model = "/home/s2425823/lora_videollama_finetuned_610v3_12_10epochs" 
# Путь к файлам настройки тренировки:
out_dir = "/home/s2425823/dog_gait_lora_610v3_12_10epochs" 

json_path = "/home/s2425823/dataset610v3/train.json" # путь к train датасету
...
class EvalSampleCallback(TrainerCallback):
    ...
    def greedy_decode(self, model, tokenizer, input_ids, attention_mask, video_frames=None, max_new_tokens=200):
        '''gready_decode работает так же как и generate(do_sample=False),
           но с возможностью вручную подать video_frames'''
        model.eval()
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        for i in range(min(self.num_samples, len(self.val_dataset))):
            ...
            # при активном gready_decode, должен быть выключен model.generate и наоборот
...
def inference():
    ...
    # Инференс или проверка на validate датасете
    for part in data:
        ...
        # Проверка итоговой обученой модели.
        inputs = processor(
        ...
        # А так же исходная сырая модель, для сравнения.
        inputs = processor(
            conversation=conversation,
            add_system_prompt=True,
            add_generation_prompt=True,
        ...
        if "pixel_values" in inputs:
            inputs["pixel_values"] = inputs["pixel_values"].to(torch.bfloat16)
        output_ids = base_model.generate(**inputs, max_new_tokens=4000)
        response = processor.batch_decode(output_ids, skip_special_tokens=True)[0].strip()
        print('\nBASE MODEL.\n')
        print("ASSISTENT:\n", response)
        del processor
...
def main():
    ...
    lora_config = LoraConfig(
        ...
        # modules_to_save=["lm_head"] # важно для генерации (?)
    )
    ...
    training_args = TrainingArguments(
        output_dir=out_dir,
        num_train_epochs=10, # 3, 30; Количество эпох (слишком много эпох переучит модель)
        per_device_train_batch_size=1, # 1 2 
        gradient_accumulation_steps=4, # 8 4
        learning_rate=5e-10, # 5e-5 или 1e-4 # Начальная скорость обучения (5e-5, 7e-5)
        ...
        # optim="adafactor",  # Экономия памяти; "adamw_torch" или "adafactor", первый более стабилен 
    ...
    # num_samples - количество сэмплов в валидационном датасете
    ...
    # Запускаем инференс с выводом в консоль результатов.
    del model
    del tokenizer
    del training_args
    del trainer
